...
- [X] T010 [P] [US2] Create `__tests__/components/ChatPanel.test.tsx` — render `<ChatPanel />` with required props via `@testing-library/react`; `toMatchSnapshot()`; assert component mounts without throwing
- [X] T011 [P] [US2] Create `__tests__/components/MessageInput.test.tsx` — render `<MessageInput />`; snapshot; assert `onChange` fires when input value changes
- [X] T012 [P] [US2] Create `__tests__/components/MessageList.test.tsx` — render with empty array (empty state) and populated array; snapshot both; assert correct message count
- [X] T013 [P] [US2] Create `__tests__/components/StreamingMessage.test.tsx` — render with partial streaming content and completed content; snapshot both states
- [X] T014 [P] [US2] Create `__tests__/components/ModelSwitcher.test.tsx` — render with mock provider list; snapshot; assert selected model label is visible
- [X] T015 [P] [US2] Create `__tests__/components/PersonaSelector.test.tsx` — render with mock personas; snapshot; assert persona names render
- [X] T016 [US2] Create `__tests__/components/ErrorBoundary.test.tsx` — wrap a child that throws in a React error boundary; assert fallback UI renders; mock `console.error` to suppress React's error output in test output
...
- [X] T019 [US2] Extend `__tests__/modules/chat/chat-service.test.ts` — add test for provider error path: mock LLM client to throw; assert `handleChatMessage` propagates or handles the error per its contract
- [X] T020 [US2] Extend `__tests__/modules/chat/chat-service.test.ts` — add test for context-window trim path: construct conversation with >20 messages or token count exceeding 100K; assert messages are trimmed before the LLM call
- [X] T021 [US2] Extend `__tests__/modules/chat/chat-service.test.ts` — add test for Zod validation failure: call route handler with a message >10,000 chars; assert 400 response with validation error body
